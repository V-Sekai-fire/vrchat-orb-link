...
namespace VoyageVoyage
{
    ...
    public class GLBLoader : UdonSharpBehaviour
    {
        ...
        Mesh[][] m_meshChunks = new Mesh[0][]; // Chunked meshes for progressive loading
        int[] m_meshChunkMaterialIndices = new int[0]; // Material indices for chunks
        ...
        void SplitMeshIntoSubmeshes(Mesh mesh)
        {
            ...
            const int trianglesPerSubmesh = 5000; // Fixed count per submesh
        ...
        public void ParseGLB()
        {
            ...
            if (!StillHaveTime())
            {
                limit = Time.realtimeSinceStartup + 0.02f; // Increased for faster total parse time
            }
